...
static lv_obj_t *btn_setup;          /* 设置按钮*/
...
static void lv_example_btn_stop(void)
{
    ...
    lv_obj_set_style_text_font(label, font, LV_PART_MAIN);                          /* 设置字体 */
    lv_label_set_text(label, "Stop1");                                               /* 设置标签文本 */
    lv_obj_set_align(label,LV_ALIGN_CENTER);                                        /* 设置标签位置 */
}

/**
 * @brief  设置按钮(这个按钮是王耀辉增加的)
 * @param  无
 * @return 无
 */
static void lv_example_btn_setup(void)
{
    btn_setup = lv_btn_create(lv_scr_act());                                         /* 创建急停按钮 */
    lv_obj_set_size(btn_setup, scr_act_width() / 4, scr_act_height() / 6);           /* 设置按钮大小 */
    lv_obj_align(btn_setup, LV_ALIGN_CENTER, 0, scr_act_height() / 3);                /* 设置按钮位置 */
    lv_obj_set_style_bg_color(btn_setup, lv_color_hex(0xef5f60), LV_STATE_DEFAULT);  /* 设置按钮背景颜色（默认） */
    lv_obj_set_style_bg_color(btn_setup, lv_color_hex(0xff0000), LV_STATE_PRESSED);  /* 设置按钮背景颜色（按下） */
    lv_obj_add_event_cb(btn_setup, btn_event_cb, LV_EVENT_CLICKED, NULL);            /* 设置按钮事件 */

    lv_obj_t* label = lv_label_create(btn_setup);                                    /* 创建急停按钮标签 */
    lv_obj_set_style_text_font(label, font, LV_PART_MAIN);                          /* 设置字体 */
    lv_label_set_text(label, "Setup");                                               /* 设置标签文本 */
    lv_obj_set_align(label,LV_ALIGN_CENTER);                                        /* 设置标签位置 */
}
...
void lv_mainstart(void)
{
    ...
    lv_example_btn_stop();          /* 急停按钮 */
		lv_example_btn_setup();         /*王耀辉增加的设置按钮*/
}
